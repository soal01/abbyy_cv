...
std::pair<double, double> findAngle(Mat &img) {
    ...
    //return {(W / 2.0 - jmax) / (W / 2.0) * 45, work_time};
}
...
int main(int argc, char** argv )
{
    ...
    //imwrite("median.bmp", image1);

    //Sobel(image1, image2, 3, 1, 1, 3);
    applySobel(image1, image2);
    //applySobel(oldImage, image2);
    imwrite("sobel.bmp", image2);
    ...
    //resize up
    //resize(oldImage, image2, Size(new_H, new_W), INTER_LINEAR);
    //resize(image2, image3, Size(new_H, new_W), INTER_LINEAR);
    ...
    Mat res_sobel;
    //my_rotate(big_sobel, res_sobel, -angle);
    //imwrite("rot_sobel.bmp", res_sobel);
    //double angle = 45;
    std::cout << "Угол поворота " << angle << std::endl;
    //Mat result(image2.rows, image2.cols, CV_64F);
    my_rotate(oldImage, result, -angle, 2);
    //my_rotate(oldImage, image1, -angle);
